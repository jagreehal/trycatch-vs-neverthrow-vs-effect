...
export function createPaymentWorkflow(
  ...
  return workflow.run(async ({ step, deps }) => {
    ...
    // 4) Call provider. On failure the step early-exits with the error,
    //    after recording it for the audit trail (fire-and-forget side effect)
    const response = await step.fromResult(
      'callProvider',
      () => deps.callProvider(provider, input),
      {
        onError: (providerError) => {
          persistFailure(db, input, providerError, actorEmail).catch(() => {
    ...
    return await step(
      'persistSuccess',
      () => deps.persistSuccess(db, input, response, actorEmail),
      {
...
describe('workflow: functional utilities (pipe/flow)', () => {
  it('composes with pipe', async () => {
    // Awaitly 4 exposes Result combinators from the root entry point.
...
describe('workflow: additional features', () => {
  it('captures events for observability', async () => {
    ...
    await workflow.run(async ({ step, deps: d }) => {
  ...
  it('demonstrates automatic error union inference', async () => {
    ...
    const result = await workflow.run(async ({ step, deps: d }) => {
    ...
    const errorResult = await workflow.run(async ({ step, deps: d }) => {
